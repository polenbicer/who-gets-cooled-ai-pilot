...
export default function Home() {
  ...
  const applySandboxPolicy = (policyType: string) => {
    ...
    // Maliyetler: Green Canopy (30), Tenant Protection (20), Cooling Hubs (15)
    ...
    // Paran yetersizse direkt oyunu kaybettir!
    if (currentBudget < requiredCost) {
      setSandboxGameStatus('lost');
      triggerSandboxAlert(`💀 Game Over! You don't have enough budget (${currentBudget} pts) to execute this policy (Requires ${requiredCost} pts). Municipality went bankrupt!`);
      return;
    }
              ...
              <div className="absolute inset-0 bg-slate-950/90 z-50 flex flex-col items-center justify-center p-6 rounded-2xl text-center backdrop-blur-sm">
                {sandboxGameStatus === 'won' ? (
                  ...
                  <>
                    ...
                    <p className="text-slate-300 max-w-md mb-6 text-sm">
                      Your municipal budget was depleted, insufficient for policy changes, or social trust collapsed. You lost the election!
                    </p>
                  </>
                )}
                <button 
